const PINPUT: &str = "nine92jnhgqzctpgbcbpz
sevensddvc73three
9986fmfqhdmq8
7onexmxbzllfqb
six777
...
fn main() {
    let mut total = 0;
    for line in PINPUT.lines() {
        let mut fi: i32 = -1;
        let mut fl: i32 = -1;
        for (i, char) in line.chars().enumerate() {
            if char.is_numeric() {
                if fi == -1 {
                    fi = char.to_digit(10).unwrap() as i32;
                }

                fl = char.to_digit(10).unwrap() as i32;
            } else if let Some(num) = is_worddigit(&line[i..], &char) {
                if fi == -1 {
                    fi = num;
                }
...
fn is_worddigit(rest: &str, char: &char) -> Option<i32> {
    match char {
        'o' => {
            if rest.len() >= 3 && rest[0..=2].eq("one") {
                Some(1)
            } else {
                None
            }
        ...
        't' => {
            if rest.len() >= 3 && rest[0..=2].eq("two") {
                Some(2)
            } else if rest.len() >= 5 && rest[0..=4].eq("three") {
                Some(3)
            } else {
                None
            }
        ...
        'f' => {
            if rest.len() >= 4 && rest[0..=3].eq("four") {
                Some(4)
            } else if rest.len() >= 4 && rest[0..=3].eq("five") {
                Some(5)
            } else {
                None
            }
        ...
        's' => {
            if rest.len() >= 3 && rest[0..=2].eq("six") {
                Some(6)
            } else if rest.len() >= 5 && rest[0..=4].eq("seven") {
                Some(7)
            } else {
                None
            }
        ...
        'e' => {
            if rest.len() >= 5 && rest[0..=4].eq("eight") {
                Some(8)
            } else {
                None
            }
        ...
        'n' => {
            if rest.len() >= 4 && rest[0..=3].eq("nine") {
                Some(9)
            } else {
                None
            }
