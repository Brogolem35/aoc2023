...
const PIN: &str = "...................15....904...........850.................329...................13....................................871....816....697....
...
impl Num {
    fn check(&self, m: &mut HashMap<(usize, usize), Gear>) {
        let y1 = self.y.checked_sub(1).unwrap_or(0);
        let y2 = self.y + 1;

        let x1 = self.x1.checked_sub(1).unwrap_or(0);
        let x2 = self.x2 + 1;

        for i in y1..=y2 {
            for j in x1..=x2 {
                if let Some(ref mut g) = m.get_mut(&(j, i)) {
                    if let None = g.0 {
                        g.0 = Some(self.val);
                    } else if let None = g.1 {
                        g.1 = Some(self.val);
                    }
                }
            }
...
#[derive(Debug)]
struct Gear(Option<i32>, Option<i32>);

impl Gear {
    fn ratio_plus_l(&self) -> i32 {
...
fn main() {
    ...
    let pmap: Vec<Vec<char>> = PIN.lines().map(|s| s.chars().collect()).collect();
    let pllen = pmap[0].len() + 1;

    for i in PIN.match_indices(SYMBOL).map(|(i, _)| i) {
        let x = i % pllen;
        let y = i / pllen;

        gears.insert((x, y), Gear(None, None));
    }

    for mat in re.find_iter(&PIN) {
        let s = mat.as_str();

        let y = mat.start() / pllen;
        let x1 = mat.start() % pllen;
        let x2 = mat.end() % pllen - 1;
        let val: i32 = s[0..].parse().unwrap();

        let num = Num { y, x1, x2, val };
    ...
    let total: i32 = gears.values().map(|g| g.ratio_plus_l()).sum();
    println!("{total}");
}
