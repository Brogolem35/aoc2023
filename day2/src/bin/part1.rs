...
const PIN: &str = "Game 1: 18 red, 8 green, 7 blue; 15 red, 4 blue, 1 green; 2 green, 17 red, 6 blue; 5 green, 1 blue, 11 red; 18 red, 1 green, 14 blue; 8 blue
...
fn main() {
    let mut total = 0;
    let rr = Regex::new(r"(?<rcnt>\d+) red").unwrap();
    let rg = Regex::new(r"(?<gcnt>\d+) green").unwrap();
    let rb = Regex::new(r"(?<bcnt>\d+) blue").unwrap();

    'li: for line in PIN.lines() {
        let tmp: Vec<&str> = line.split(":").collect();
        let num: i32 = tmp[0][5..].parse().unwrap();
        let sets: Vec<&str> = tmp[1].split(";").collect();

        for (i, set) in sets.iter().enumerate() {
            let rcnt: i32 = if let Some(cap) = rr.captures(set) {
                cap["rcnt"].parse().unwrap()
            } else {
                0
            };

            let gcnt: i32 = if let Some(cap) = rg.captures(set) {
                cap["gcnt"].parse().unwrap()
            } else {
                0
            };

            let bcnt: i32 = if let Some(cap) = rb.captures(set) {
                cap["bcnt"].parse().unwrap()
            } else {
                0
            };

            if rcnt > MAX_RED || gcnt > MAX_GREEN || bcnt > MAX_BLUE {
                dbg!(num, i);
                continue 'li;
            }
        }
